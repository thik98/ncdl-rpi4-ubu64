...
def ffmpeg_cnv_minivideo(inputfilename , outputfilename):
    # ffmpegコマンドライン文字列作成
    cmd = 'ffmpeg -y -analyzeduration 10MB -probesize 10MB -i "{0}" -vf \'scale=768:-2\' -c:v h264_v4l2m2m -b:v 1M -c:a libfdk_aac -b:a 192k "{1}"'.format(inputfilename , outputfilename)
    # コマンドを実行する
